...
router.post('/signin',(req, resp)=>{
    ...
    if(!email || !password){
        return resp.status(422).json({error:"please add email or password"})
    }
    User.findOne({email:email})
    .then((savedUser)=>{
        if(!savedUser){
            return resp.status(422).json({error:"Invalid user"});
        }

        bcrypt.compare(password, savedUser.password)
        .then(doMatch=>{
            if(doMatch){
                // resp.json({message:"successfully logged in"}); 
                const token = jwt.sign({_id:savedUser._id}, JWT_SECRET)
                const {_id, name, email, followers, following, pic} = savedUser
                resp.json({token, user:{_id, name, email,followers,following,pic}})
            }
            else{
